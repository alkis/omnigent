...
must pass ``include_items=false&include_liveness=false&include_live_status=false``.
A bare read makes the server build the full page — transcript items, liveness,
and on a status-cache miss a live-status probe of the very runner performing
the read while it waits on the answer.
"""
...
# is these exact three pairs, and the test must keep failing if they change.
...
async def test_runner_owned_session_reads_pass_trim_params(
    ...
    """Every runner-owned session read carries the three trim flags."""
