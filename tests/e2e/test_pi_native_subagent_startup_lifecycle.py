"""Real Pi startup, steering, and the default 180-second launch watchdog.
...
The old startup idle completes the initial dispatch while bash is still
running. A public MCP follow-up becomes a new launching dispatch and times
out after 180 seconds, although the same Pi/tool processes remain alive.
...
pytestmark = pytest.mark.timeout(480, method="signal")
...
def test_real_pi_child_startup_cannot_complete_or_timeout_running_work(
    ...
    try:
        ...
        watchdog_failure_observed = any(
            "produced no activity within 180s of dispatch" in text
