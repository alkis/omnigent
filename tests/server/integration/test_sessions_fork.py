...
async def _fork_session(
    ...
    *,
    ...
    """
    Fork a session and return the fork's ``SessionResponse`` body.

    Fork is synchronous by default (no ``async_operation_id``): the POST
    materializes the clone inline and returns 201 with the finished session.

    :param client: The test HTTP client.
    :param source_id: Session ID to fork.
    :param title: Optional title for the fork.
    :returns: The forked session's JSON body.
    """
...
async def test_fork_empty_session(
    ...
    fork = await _fork_session(client, session["id"])
...
async def test_fork_preserves_labels(
    ...
    fork = await _fork_session(client, session["id"], title="My Fork")
...
async def test_fork_coding_session_stamps_fork_source_label(
    ...
    fork = await _fork_session(client, source["id"])
...
async def test_failed_fork_leaves_no_ghost_in_builtin_agents(
    ...
    # Drive the ASYNC path (async_operation_id set): a stale up_to_response_id
    # makes the store raise, which the background op turns into a
    # fork_status:"failed" — the async failure surface the Web UI relies on.
    fork_before = set(_fork_tasks)
    resp = await client.post(
        f"/v1/sessions/{session['id']}/fork",
        json={"up_to_response_id": "resp_does_not_exist", "async_operation_id": "op-ghost"},
    )
...
async def test_fork_a_fork(
    ...
    # First fork.
    fork1 = await _fork_session(client, session["id"])

    # Second fork — forking the fork.
    fork2 = await _fork_session(client, fork1["id"])
