...
def _fork_and_resolve(
    app: FastAPI,
    source_id: str,
    *,
    headers: dict[str, str] | None = None,
) -> str:
    """Synchronously fork (201) and return the fork's id.

    Fork is synchronous by default (no ``async_operation_id``): the POST
    materializes the clone inline and returns the finished session.
    """
    resp = TestClient(app).post(f"/v1/sessions/{source_id}/fork", json={}, headers=headers or {})
...
async def test_fork_inherits_source_project(db_uri: str, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    fork_id = _fork_and_resolve(app, conv.id)
...
async def test_fork_of_shared_session_in_foreign_project_stays_unfiled(
    ...
    fork_id = _fork_and_resolve(app, conv_id, headers=_hdr(ALICE))
...
async def test_fork_keeps_own_project_multi_user(
    ...
    fork_id = _fork_and_resolve(app, conv_id, headers=_hdr(BOB))
