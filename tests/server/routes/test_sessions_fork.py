...
async def _post_fork(
    ...
    """POST the fork endpoint synchronously (the default — no
    ``async_operation_id``). The materialization runs inline, so its effects
    (fork_calls, files, launches, announcements) and the 201 ``SessionResponse``
    body are observable the moment the call returns — no draining needed.

    Driven on the test's own loop via an in-loop async client so a body that
    schedules a background managed launch is awaited deterministically.
    """
    transport = ASGITransport(app=app)
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        return await client.post(
            f"/v1/sessions/{source_id}/fork", json=json or {}, headers=headers
        )


async def _post_fork_async(
    app: FastAPI, source_id: str, json: dict[str, Any], op_id: str
) -> httpx.Response:
    """POST the fork endpoint in ASYNC mode (``async_operation_id`` set) and
    drain the background materialization so its effects are observable. Returns
    the 202 accept response. Only for the tests that specifically cover the
    async path the Web UI uses."""
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post(
            f"/v1/sessions/{source_id}/fork", json={**json, "async_operation_id": op_id}
        )
    # Drain only the task(s) THIS POST spawned.
...
async def test_fork_session_happy_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    resp = await _post_fork(app, "e9f8f58523cec9a57d3bdf93be543e8c", {"title": "My Fork"})

    # Synchronous default: the fork is materialized inline and the finished
    # session is returned (201).
    assert resp.status_code == 201, f"Expected 201 Created, got {resp.status_code}: {resp.text}"
    body = resp.json()
    assert body["id"] == _FORK_ID
    ...
    ] == ["Hello", "World"], "201 body should carry the copied items"
...
@pytest.mark.asyncio
async def test_fork_async_opt_in_returns_202_and_materializes(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """With ``async_operation_id`` set (the Web UI path) the POST returns 202
    echoing that id, then the background op materializes the fork and publishes
    ``ready`` with its id — no synchronous session body."""
    conv = _make_conversation()
    conv_store = _ConversationStore(
        conversations={"e9f8f58523cec9a57d3bdf93be543e8c": conv},
        items_by_conv={
    ...
    resp = await _post_fork_async(app, "e9f8f58523cec9a57d3bdf93be543e8c", {}, "op-web-123")

    assert resp.status_code == 202, f"got {resp.status_code}: {resp.text}"
    assert resp.json() == {
        "operation_id": "op-web-123",
        "source_id": "e9f8f58523cec9a57d3bdf93be543e8c",
        "status": "cloning",
    }
    # cloning → ready, with the materialized fork's id; the fork is committed.
    assert ("cloning", None) in statuses
    assert any(status == "ready" for status, _ in statuses), statuses
    assert _FORK_ID in conv_store._convs
    assert len(conv_store.fork_calls) == 1
...
async def test_fork_background_failure_leaves_no_session(
    ...
    resp = await _post_fork_async(app, "e9f8f58523cec9a57d3bdf93be543e8c", {}, "op-boom")

    assert resp.status_code == 202, f"got {resp.status_code}: {resp.text}"
    # cloning was seeded, then the op failed. The client-facing error is the
    # GENERIC message — the raw "disk full" is only logged, never leaked.
    assert ("cloning", None) in statuses
    failed = [error for status, error in statuses if status == "failed"]
    assert failed and failed[0] == "Couldn't clone the session. Try again.", statuses
    assert not any("disk full" in (err or "") for _s, err in statuses), (
        "raw exception detail must not reach the client-facing status"
    )
...
async def test_fork_session_shares_source_file_blob_without_copying_bytes() -> None:
    ...
    resp = await _post_fork(app, source_id, {})
...
async def test_fork_session_run_config_omitted_inherits() -> None:
    ...
    resp = await _post_fork(app, "e9f8f58523cec9a57d3bdf93be543e8c", {})
...
async def test_fork_session_up_to_response_id_passes_through_and_truncates() -> None:
    ...
    # Only resp_001's two items survive the truncation in the 201 body;
    # resp_002 appearing means the store ignored the cutoff.
    assert [item["response_id"] for item in resp.json()["items"]] == ["resp_001", "resp_001"], (
        f"Fork should contain only resp_001 items, got {resp.json()['items']!r}"
    )
...
async def test_fork_accept_carries_no_transcript() -> None:
    ...
    resp = await _post_fork_async(app, "e9f8f58523cec9a57d3bdf93be543e8c", {}, "op-notrans")
...
async def test_fork_unknown_up_to_response_id_fails_in_background(
    ...
    resp = await _post_fork_async(
        app, "e9f8f58523cec9a57d3bdf93be543e8c", {"up_to_response_id": "resp_nope"}, "op-badresp"
    )

    assert resp.status_code == 202, f"got {resp.status_code}: {resp.text}"
    # The background op reported failure (generic client-facing message); no
    # fork materialized (the ValueError aborted before the fork exists).
    assert ("cloning", None) in statuses
    failed = [error for status, error in statuses if status == "failed"]
    assert failed and failed[0] == "Couldn't clone the session. Try again.", statuses
...
@pytest.mark.asyncio
async def test_fork_sync_unknown_up_to_response_id_is_400() -> None:
    """A stale ``up_to_response_id`` on a SYNCHRONOUS fork is a 400, not a 500.

    The store raises ValueError for an unknown response id; the synchronous
    path (no ``async_operation_id``) must translate it to INVALID_INPUT so a
    CLI/SDK caller gets an actionable 400 — a regression here would surface the
    store error as an uncaught 500.
    """
    conv = _make_conversation()
    conv_store = _ConversationStore(
        conversations={"e9f8f58523cec9a57d3bdf93be543e8c": conv},
    ...
    assert resp.status_code == 400, f"got {resp.status_code}: {resp.text}"
    assert _FORK_ID not in conv_store._convs, "a rejected fork must leave no committed session"
...
async def test_fork_managed_inherits_source_repository(
    ...
    resp = await _post_fork(app, "e9f8f58523cec9a57d3bdf93be543e8c", {"host_type": "managed"})
...
async def test_fork_never_inherits_source_sandbox_repo_label(
    ...
    external = await _post_fork(app, "e9f8f58523cec9a57d3bdf93be543e8c", {})
