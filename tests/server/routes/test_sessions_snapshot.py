...
@pytest.mark.asyncio
async def test_session_snapshot_does_not_claim_agent_model_for_spec_embedded_acp(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A spec whose own harness is the generic ACP wrap also owns its model."""
    spec = AgentSpec(
        spec_version=1,
        name="grok_probe",
        executor=ExecutorSpec(
            config={"harness": "acp", "acp_agent": {"name": "Grok Build", "command": "grok"}},
            model="claude-opus-5",
            context_window=1_000_000,
        ),
    )
    conv = Conversation(
        ...
        created_at=1,
        updated_at=1,
        root_conversation_id="conv_acp_spec",
        agent_id="ag_grok_probe",
    )
    store = _ConversationStore([], conversations={conv.id: conv})
