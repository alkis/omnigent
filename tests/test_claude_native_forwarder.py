...
async def test_forward_session_cost_tags_display_advance_with_model(
    ...
    """Tag display-cost growth with model attribution; policy-only growth has no new cost.

    Use the statusLine model without sub-agents and transcript model weights
    with sub-agents, so the usage panel can attribute each display advance."""
    bridge_dir = tmp_path / "bridge"
    bridge_dir.mkdir()
    parent = tmp_path / "sess.jsonl"
    parent.write_text("parent", encoding="utf-8")
