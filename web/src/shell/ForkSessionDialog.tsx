...
export function ForkSessionForm({
  ...
  async function handleFork(): Promise<void> {
    ...
    try {
      ...
      // Async fork: the CLIENT owns the operation id and registers its pending
      // state (deferred runner bind + "Cloning…" toast) BEFORE sending, so a
      // fast `ready` event can't race the registration and lose the binding.
      // The server echoes this id and drives the fork_status stream off it; the
      // session enters the sidebar and the toast flips to success/failure when
      ...
      try {
        ...
        await forkSession(sourceSessionId, {
      ...
      // still open, so surface it inline and keep inputs editable.
      setError(e instanceof Error ? e.message : "Couldn't clone the session. Try again.");
    } finally {
      setSubmitting(false);
    }
