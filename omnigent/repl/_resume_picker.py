...
async def pick_conversation_by_wrapper_label_from_sdk(
    ...
    *,
    ...
    :param host_id: When set, keep only rows running on this host (the
        ...
        dead end in this picker — resuming it cannot work here. Child
        sessions use their inherited ``routing_host_id``. Rows
    ...
    convos = [
        ...
        and (
            host_id is None
            or (getattr(c, "routing_host_id", None) or getattr(c, "host_id", None))
            in (None, host_id)
        )
    ]
    previews = await _collect_previews_async(client, convos)
