...
class SessionForkRequest(BaseModel):
    ...
    # Opt-in asynchronous fork. When set, the POST returns 202 immediately with
    # this operation id and the heavy transcript copy runs in a background task;
    # progress rides a ``fork_status`` event on the caller's session-updates
    # stream, keyed by this id, and the fork enters the sidebar only when ready.
    # The Web UI generates the id and registers its pending state BEFORE sending
    # (so a fast ``ready`` can't race the registration). Omitted (the default)
