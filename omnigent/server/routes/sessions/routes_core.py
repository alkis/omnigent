...
def register_core_routes(
    ...
    *,
    ...
    async def fork_session(
        ...
        # The transcript deep-copy + FTS rebuild below is the heavy work (a
        # large source took 66s). It runs synchronously by default (201 +
        # SessionResponse) — the contract the CLI, native hook, and SDK/REPL
        # depend on. Only when the caller opts in with async_operation_id (the
        # Web UI, which registers its pending state before sending) does it run
        # in a background task, returning 202 immediately and surfacing progress
        # via a fork_status event keyed by that id, with the destination
        # announced only on success (no ghost sidebar row mid-clone).
        op_id = body.async_operation_id
        ...
        # Synchronous default: materialize inline and return the finished
        # session (201), the contract every non-web client relies on. Translate
        # the store's typed failures to the same 4xx the pre-async endpoint
        # returned — a stale up_to_response_id is a client error, not a 500.
        if op_id is None:
            try:
        ...
        async def _run_fork(operation_id: str) -> None:
            ...
            publish a durable ``failed`` the client can retry. The client-facing
            error is generic — the raw exception (which may carry SQL/params) is
